...
# Layer 1
W1 = tf.Variable(tf.truncated_normal([layer1_filter_size, layer1_filter_size, input_channels, layer1_depth], stddev=0.1))
b1 = tf.Variable(tf.constant(0.1, shape=[layer1_depth]))
conv1 = tf.nn.conv2d(x, W1, [1, layer1_stride, layer1_stride, 1], padding='SAME') + b1
layer1_out = tf.nn.relu(conv1)

# Layer 2
W2 = tf.Variable(tf.truncated_normal([layer2_filter_size, layer2_filter_size, layer1_depth, layer2_depth], stddev=0.1))
b2 = tf.Variable(tf.constant(0.1, shape=[layer2_depth]))
conv2 = tf.nn.conv2d(layer1_out, W2, [1, layer2_stride, layer2_stride, 1], padding='SAME') + b2
layer2_out = tf.nn.relu(conv2)

# Layer 3
layer2_out_shape = layer2_out.get_shape().as_list()
layer2_out_dim = layer2_out_shape[1] * layer2_out_shape[2] * layer2_out_shape[3]
...
if __name__ == '__main__':
  X_lattices = np.load('../data/datasets/X_2d_lattices.npy')
